...
/**
 * A ripped internet-radio broadcast, transcribed by a machine rather
 * than retyped by a person -- so unlike a source item, this should NOT
 * read as clean prose. Monospace, a fake file/status header like a
 * ripped audio clip, and the rough transcription quality left entirely
 * alone (no smoothing over the machine's mistakes -- that roughness IS
 * the artifact: real static-y found audio, not a polished document).
 */
function renderTranscript(item) {
  const bodyHtml = textToParagraphs(item.text, `font-family:'Courier New',monospace;`);
  return `
  <div style="background:#111111;padding:34px 14px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;">
      <tr>
        <td style="background:#0d0d0d;border:1px solid #333;padding:26px 26px 20px;">
          <p style="margin:0 0 18px;font-family:'Courier New',monospace;font-size:11px;letter-spacing:0.1em;color:#5a8f5a;">
            &#9679;REC &nbsp; darkshow.mp3 &nbsp; [auto-transcribed, uncorrected]
          </p>
          <div style="font-family:'Courier New',monospace;font-size:13px;line-height:1.8;color:#c7c7c7;">
            ${bodyHtml}
          </div>
        </td>
      </tr>
...
function buildOngshatEmailHtml({ item, siteUrl, total }) {
  ...
  if (item.type === 'transcript') return renderTranscript(item);
...
function buildSubject(item, total) {
  ...
  return '.'; // notes, images, and transcripts all arrive bare, unexplained
}
