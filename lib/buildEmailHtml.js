...
// wrapShell used to add a badge ("LIVEJOURNAL ENTRY"), the sender name,
// a "Dionaea House -- Part N" line, and a date footer around EVERY
// type's own already-complete recreation (the LJ purple header already
// says it's LiveJournal; the AIM window already looks like AIM; the
// Outlook-style email window already has its own From/Date/Subject).
// That outer layer was pure redundant meta-chrome sitting on top of a
// self-sufficient recreation, and it's what was breaking the illusion --
// so this is now just a plain container: white background, a sensible
// max-width, done. What renders inside IS the whole message.
function wrapShell({ innerHtml }) {
  return `
